...
const copy = {
  en: {
    ...
    wallet: {
      ...
      cardSmalls: ["KYC status without wallet exposure", "Prove enough, not the exact amount"],
      requestLabel: "DApp request",
      requestBody: "Prove KYC status or balance eligibility for this action.",
      requestButton: "Generate proof",
      disclosureTitle: "Shared with dApp",
      disclosures: ["KYC: verified ✓", "Nullifier: scoped", "Wallet address: hidden", "Balance: hidden"],
      stepTitles: [
        "User receives Dojang credentials",
        "dApp requests the needed proof",
        "Wallet explains the disclosure",
        "Proof is generated",
        "dApp verifies and applies policy",
      ],
      stepBodies: [
        "ZKProofport surfaces Verified Address and balance-related credentials, with a path to GIWA Wallet-native UX.",
        "The SDK request specifies KYC status, balance threshold, issuer, scope, and verifier.",
        "The user sees exactly what will be revealed and what stays private.",
        "On-device for humans, with an agent path available for automated workflows.",
        "Access, one-user-one-action, eligibility, or threshold logic is enforced.",
      ],
    },
    build: {
      eyebrow: "Build scope",
      ...
      note:
        "We will ship a narrow end-to-end flow first: privacy-preserving KYC proof, GIWA verifier, SDK request, and one showcase dApp. Balance threshold proof follows the same model as the next expansion.",
    ...
    traction: {
      ...
      title: "We are adapting working infrastructure, not starting from zero.",
    ...
    team: {
      ...
      names: ["Hyunsu Young", "Jehyuk Hyun"],
      bodies: [
      ...
      closeBody:
        "We want ZKProofport to become the privacy layer that makes Dojang more useful for GIWA users and easier for GIWA dApp builders to adopt. As GIWA grows toward regulated finance — including KRW stablecoins and RWA — the ability to prove compliance without exposing personal data becomes essential. GASOK gives us the alignment, feedback, and integration surface to build this as a GIWA-native product.",
    },
  },
  ko: {
    meta: {
    ...
    wallet: {
      eyebrow: "지갑 흐름과 아키텍처",
      title: "dApp은 SDK를 사용하고, 사용자는 현재 앱에서 증명하며 이후 wallet-native 흐름으로 확장합니다.",
      screenTitle: "Private Dojang Pass",
      cardLabels: ["Verified Address", "Verified Balance"],
      cardNames: ["Upbit Korea", "임계값 증명"],
      cardSmalls: ["지갑 주소 노출 없이 KYC 상태 증명", "정확한 잔고가 아니라 조건 충족 여부만 증명"],
      requestLabel: "dApp 요청",
      requestBody: "이 액션에 필요한 KYC 상태 또는 잔고 조건을 증명합니다.",
      requestButton: "증명 생성",
      disclosureTitle: "dApp에 공유되는 정보",
      disclosures: ["KYC: 인증됨 ✓", "Nullifier: 목적별 생성", "지갑 주소: 미공개", "잔고: 미공개"],
      stepTitles: [
        "사용자가 Dojang 인증 정보를 받습니다",
        "dApp이 필요한 증명을 요청합니다",
        "지갑이 공개 범위를 설명합니다",
        "증명이 생성됩니다",
        "dApp이 검증하고 정책을 적용합니다",
      ],
      stepBodies: [
        "ZKProofport에서 Verified Address와 Verified Balance 관련 인증 정보를 확인하고, 이후 GIWA Wallet-native UX로 확장할 수 있습니다.",
        "SDK 요청에는 KYC 여부, 잔고 기준, 발행자, 목적, 검증기 정보가 포함됩니다.",
        "사용자는 dApp에 무엇이 전달되고, 어떤 정보가 보호되는지 확인합니다.",
        "일반 사용자는 기기 안에서 증명을 생성하고, 자동화된 흐름에서는 에이전트 경로를 사용할 수 있습니다.",
        "dApp은 접근 권한, 1인 1회 참여, 자격 확인, 잔고 기준 충족 여부를 적용합니다.",
      ],
    },
    build: {
      eyebrow: "구축 범위",
    ...
    roadmap: {
      ...
      bodies: [
        ...
        "Korea Blockchain Week에서 라이브 데모: 앱 기반 증명, 지갑 네이티브 경로, 잔고 임계값 증명 미리보기.",
      ],
    },
    team: {
      eyebrow: "팀",
...
const bindings = [
  ...
  { selector: ".wallet-title", key: "wallet.screenTitle" },
  { selector: ".wallet-card span", key: "wallet.cardLabels", all: true },
  { selector: ".wallet-card b", key: "wallet.cardNames", all: true },
  { selector: ".wallet-card small", key: "wallet.cardSmalls", all: true },
  { selector: ".wallet-request span", key: "wallet.requestLabel" },
  { selector: ".wallet-request p", key: "wallet.requestBody" },
  { selector: ".wallet-request button", key: "wallet.requestButton" },
  { selector: ".wallet-disclosure b", key: "wallet.disclosureTitle" },
  { selector: ".wallet-disclosure span", key: "wallet.disclosures", all: true },
  { selector: ".flow-steps b", key: "wallet.stepTitles", all: true },
  { selector: ".flow-steps span", key: "wallet.stepBodies", all: true },
